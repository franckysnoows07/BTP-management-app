...
#All Variables =================
 ...
 #=====title======
        title=Label(self.root,text="Infos Perdieme",font=("goudy old style",15),bg="#0f4d7d",fg="white").pack(side=TOP,fill=X)
    ...
    def show(self):
        ...
        cur=con.cursor()
        try:
            cur.execute("SELECT * FROM perdieme")
            rows=cur.fetchall()
    ...
    def add(self):
        ...
        try:
            ...
            cur.execute("Insert into perdieme (date,conducteur,heured,heurep,heurer,heuref,heuret,heureta,montant) values(?,?,?,?,?,?,?,?,?)",(
                                        self.var_date.get(),
                                        self.var_name.get(),
                                        self.var_hours_bgn,
                                        self.var_hours_pse,
    ...
    def delete(self):
        ...
        try:
                    ...
                    if op ==True:
                        cur.execute("delete from perdieme where date=? and conducteur=?",(self.var_date.get(),self.var_name.get(),))
                        con.commit()
        ...
        try:
            if self.var_searchby.get() == " ":
                ...
                if self.var_searchby.get() in valid_columns:
                    ...
                    query = f"SELECT * FROM perdieme WHERE {self.var_searchby.get()} LIKE ?"
