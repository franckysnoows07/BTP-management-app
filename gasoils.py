...
#All Variables =================
    ...
    #=====title======
        title=Label(self.root,text="Infos Gasoil",font=("goudy old style",15),bg="#0f4d7d",fg="white").pack(side=TOP, fill=X)


    #=====content=====
    #=====ligne premier====
        gasoil_frame = Frame(self.root, bd=2, relief=RIDGE, bg="white")
        gasoil_frame.place(x=10, y=40, width=290, height=450)

    #======name selector =====  
        lbl_cond_name = Label(gasoil_frame, text="Conducteur", font=("goudy old style", 15), bg="white").place(x=0, y=5)
        self.cmb_cond_name = ttk.Combobox(gasoil_frame, textvariable=self.var_cond_surname, font=("goudy old style", 12), state='readonly')
    ...
    #======machine selector=========
        lbl_machine = Label(gasoil_frame, text="Machine", font=("goudy old style", 15), bg="white").place(x=0, y=40)
        self.cmb_machine = ttk.Combobox(gasoil_frame, textvariable=self.var_machine_name, font=("goudy old style", 10), state='readonly')
        self.cmb_machine.place(x=130, y=40, width=150)
        # Populate the combobox with conducteur names from the database
        self.populate_machine_names()

        lbl_mois = Label(gasoil_frame, text="Mois", font=("goudy old style", 15), bg="white").place(x=0, y=80)
        self.cmb_mois = ttk.Combobox(gasoil_frame, font=("goudy old style", 12), textvariable=self.var_mois, state='readonly')
        ...
        lbl_date = Label(gasoil_frame, text="Date", font=("goudy old style", 15), bg="white").place(x=0, y=120)
        date_entry = DateEntry(gasoil_frame, textvariable=self.var_date, date_pattern='dd/mm/yyyy', width=18, background='darkblue', foreground='white', borderwidth=2, locale='fr_FR')
        date_entry.place(x=130, y=120, width=150, height=30)

    #=======capacity=====
        lbl_capacity_bgn=Label(gasoil_frame,text="Gasoil initial",font=("goudy old style", 15), bg="white").place(x=0, y=160)
        txt_capacity_bgn=Entry(gasoil_frame, textvariable=self.var_capacity_bgn, font=("goudy old style",15), bg="lightyellow").place(x=130, y=160, width=150)

        lbl_capacity_add=Label(gasoil_frame,text="Gasoil ajouté",font=("goudy old style", 15), bg="white").place(x=0, y=200)
        txt_capacity_add=Entry(gasoil_frame, textvariable=self.var_capacity_add, font=("goudy old style",15), bg="lightyellow").place(x=130, y=200, width=150)

        # lbl_capacity_rst=Label(gasoil_frame,text="Gasoil restant",font=("goudy old style", 15), bg="white").place(x=0, y=240)
        # txt_capacity_rst=Entry(gasoil_frame, textvariable=self.var_capacity_rst, font=("goudy old style",15), bg="lightyellow").place(x=130, y=240, width=150)

        lbl_id=Label(gasoil_frame,text="ID ",font=("goudy old style", 15), bg="white").place(x=0, y=280)
        txt_id=Entry(gasoil_frame, textvariable=self.var_gas_id, font=("goudy old style",15), bg="lightyellow").place(x=70, y=280, width=50)


    #========buttons===========
        btn_save=Button(gasoil_frame,text="Sauvegarder",command=self.add,font=("goudy old style",12),bg="#2196f3",fg="white",cursor="hand2").place(x=70,y=310,width=100,height=30)
        btn_clear=Button(gasoil_frame,text="Effacer",command=self.clear,font=("goudy old style",12),bg="#607d8b",fg="white",cursor="hand2").place(x=190,y=310,width=90,height=30)
        btn_update=Button(gasoil_frame,text="Maj",command=self.update,font=("goudy old style",12),bg="#4caf50",fg="white",cursor="hand2").place(x=70,y=345,width=100,height=30)
        btn_suppr=Button(gasoil_frame,text="Supprimer",command=self.delete,font=("goudy old style",12),bg="#f44336",fg="white",cursor="hand2").place(x=190,y=345,width=90,height=30)
    ...
    #========Gasoil Details Frame
        ...
        self.GasTable.heading("capacityd",text="Gasoil Début")
        self.GasTable.heading("capacitya",text="Gasoil Ajouté")
        self.GasTable.heading("capacityr",text="Gasoil Restant")
        ...
        self.ConsumptionTable.heading("consumption", text="Gasoil ajouté")
        ...
        self.MachTable.heading("consumption", text="Gasoil ajouté")
        ...
        self.MoisTable.heading("consumption", text="Gasoil ajouté")
    ...
    def populate_consumption_table(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("SELECT  date, SUM(capacitya) as consumption FROM gasoil GROUP BY date")
            rows = cur.fetchall()
    ...
    def populate_mach_table(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("SELECT  machine_name, SUM(capacitya) as consumption FROM gasoil GROUP BY machine_name")
            rows = cur.fetchall()
    ...
    def populate_mois_table(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("SELECT  mois, SUM(capacitya) as consumption FROM gasoil GROUP BY mois")
            rows = cur.fetchall()
    ...
    def add(self):
        ...
        cur=con.cursor()
        try:
            cur.execute("SELECT capacityr FROM gasoil ORDER BY id DESC LIMIT 1")
            ...
            cur.execute("Insert into gasoil (mois,date,nom,machine_name,capacityd,capacitya,capacityr) values(?,?,?,?,?,?,?)",(
                                        #self.var_gas_id.get(),
                                        self.var_mois.get(),
                                        self.var_date.get(),
    ...
    def show(self):
        ...
        cur=con.cursor()
        try:
            cur.execute("SELECT mois,date,nom,machine_name,capacityd,capacitya,capacityr FROM gasoil ORDER BY id")
            rows=cur.fetchall()
    ...
    def show2(self):
        ...
        cur = con.cursor()
        try:
            cur.execute("SELECT date, SUM(capacitya) as consumption FROM gasoil GROUP BY date ORDER BY date ASC")
            rows = cur.fetchall()
            self.ConsumptionTable.delete(*self.ConsumptionTable.get_children())
            for row in rows:
                self.ConsumptionTable.insert('', END, values=row)
    ...
    def get_data(self,ev):
        ...
        #self.var_capacity_csm.set(row[8])
        ...
        try:
            if self.var_searchby.get() == " ":
                ...
                if self.var_searchby.get() in valid_columns:
                    ...
                    query = f"SELECT * FROM gasoil WHERE {self.var_searchby.get()} LIKE ?"
    ...
    def update(self):
        ...
        try:
            ...
            #     self.var_capacity_csm = float(self.var_capacity_bgn.get()) + float(self.var_capacity_add.get()) - float(self.var_capacity_rst.get())
                cur.execute("Update  gasoil set  mois=?, date=?, nom=?, machine_name=?, capacityd=?, capacitya=?, capacityr=?  where nom=? and capacityr=? ",(
                                            self.var_mois.get(),
                                            self.var_date.get(),
                                            self.var_cond_surname.get(),
                                            self.var_machine_name.get(),
    ...
    # Add this method to your gasoilsClass
    ...
    def delete(self):
        ...
        try:
            ...
            #     else:
                    ...
                    if op ==True:
                        cur.execute("delete from gasoil where nom=? and capacityr=?  ",(self.var_cond_surname.get(),self.var_capacity_rst.get(),))
                        con.commit()
